...
// DBList 获取数据库列表
func (s *sToolsGenTable) DBList(ctx context.Context, req *system.ToolsGenDBReq) (res *system.ToolsGenDBRes, err error) {
	res = new(system.ToolsGenDBRes)
	res.List = make([]string, 0)
	val, err := g.Cfg().Get(ctx, "database")
	liberr.ErrIsNil(ctx, err, "获取数据库失败")
	databases := val.Map()
	for k := range databases {
		if k == "logger" {
			continue
		}
		res.List = append(res.List, k)
	}
	sort.Stable(sort.StringSlice(res.List))
	return
}
...
func (s *sToolsGenTable) SelectDbTableList(ctx context.Context, req *system.ToolsGenTableImportSearchReq) (res *system.ToolsGenTableSearchRes, err error) {
	...
	db := g.DB(req.Group)
	err = g.Try(ctx, func(ctx context.Context) {
		if !s.IsMysql(req.Group) && !s.IsPg(req.Group) && !s.IsDM(req.Group) {
			liberr.ErrIsNil(ctx, gerror.New("代码生成暂时只支持mysql、postgresql及达梦数据库"))
		}
		var sqlStr string
		if s.IsMysql(req.Group) {
			sqlStr = " from information_schema.tables where table_schema = (select database())"
			if s.IsDefault(req.Group) {
				sqlStr += " and table_name NOT LIKE 'tools_gen_%' and table_name NOT IN (select table_name from " + dao.ToolsGenTable.Table() + ") "
			} else {
				sqlStr += " and table_name NOT LIKE 'tools_gen_%'"
			}
		...
		} else if s.IsPg(req.Group) {
			sqlStr = " from information_schema.tables JOIN pg_class ON pg_class.relname = table_name and pg_class.relnamespace = (select oid  from pg_namespace where nspname=current_schema())"
			if s.IsDefault(req.Group) {
				sqlStr += " WHERE table_schema = current_schema() AND table_name NOT LIKE 'tools_gen_%' and table_name NOT IN (select table_name from " + dao.ToolsGenTable.Table() + ") "
			} else {
				sqlStr += " WHERE table_schema = current_schema() AND table_name NOT LIKE 'tools_gen_%'"
			}
			...
			if s.IsDefault(req.Group) {
				sqlStr += " AND T.TABLE_NAME NOT IN (SELECT TABLE_NAME FROM TOOLS_GEN_TABLE) "
			}
			if req != nil {
				if req.TableName != "" {
...
func (s *sToolsGenTable) SelectDbTableListByNames(ctx context.Context, tableNames []string, group ...string) ([]*entity.ToolsGenTable, error) {
	var result []*entity.ToolsGenTable
	err := g.Try(ctx, func(ctx context.Context) {
		if !s.IsMysql(group...) && !s.IsPg(group...) && !s.IsDM(group...) {
			liberr.ErrIsNil(ctx, gerror.New("代码生成暂时只支持mysql、postgresql及达梦数据库"))
		}
		db := g.DB(group...)
		var sqlStr string
		...
		} else if s.IsPg(group...) {
			sqlStr = "select information_schema.tables.*,obj_description(pg_class.oid) AS table_comment " +
				" from information_schema.tables JOIN pg_class ON pg_class.relname = table_name and pg_class.relnamespace = (select oid  from pg_namespace where nspname=current_schema())" +
		...
		err := g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
			err := g.Try(ctx, func(ctx context.Context) {
				for _, table := range tableList {
					...
					// 保存列信息
					genTableColumns, err := service.ToolsGenTableColumn().SelectDbTableColumnsByName(ctx, tableName, table.Group)
...
// 获取数据库驱动类型
func (s *sToolsGenTable) getDbDriver(group ...string) string {
	if len(group) == 0 {
		group = []string{"default"}
	}
	config := g.DB(group[0]).GetConfig()
	return gstr.ToLower(config.Type)
}

func (s *sToolsGenTable) IsPg(group ...string) bool {
	return s.getDbDriver(group...) == "pgsql"
}

func (s *sToolsGenTable) IsMysql(group ...string) bool {
	return s.getDbDriver(group...) == "mysql"
}

func (s *sToolsGenTable) IsDM(group ...string) bool {
	return s.getDbDriver(group...) == "dm"
}

func (s *sToolsGenTable) IsDefault(group string) bool {
...
func (s *sToolsGenTable) SaveEdit(ctx context.Context, req *system.ToolsGenTableColumnsEditReq) (err error) {
	...
	if req.Attachments != nil {
		table.Attachments = req.Attachments
	}
...
func (s *sToolsGenTable) removeForeignKeyColumns(columns []*model.ToolsGenTableColumnEx, foreignKey string) []*model.ToolsGenTableColumnEx {
	var newColumns []*model.ToolsGenTableColumnEx
	for _, column := range columns {
		if column.ColumnName != foreignKey {
...
// tableId 表格id
// isAttr 是否副表
func (s *sToolsGenTable) GenData(ctx context.Context, tableId int64, attr *entity.Attachments) (data g.MapStrStr, extendData *model.ToolsGenTableEx, err error) {
	...
	if attr != nil {
		extendData.IsAttachment = true
		// 移除查询参数中的外键字段
		extendData.QueryColumns = s.removeForeignKeyColumns(extendData.QueryColumns, attr.ForeignKey)
		// 移除详情参数中的外键字段
		extendData.DetailColumns = s.removeForeignKeyColumns(extendData.DetailColumns, attr.ForeignKey)
		// 移除编辑参数中的外键字段
		extendData.EditColumns = s.removeForeignKeyColumns(extendData.EditColumns, attr.ForeignKey)
		// 移除列表参数中的外键字段
		extendData.ListColumns = s.removeForeignKeyColumns(extendData.ListColumns, attr.ForeignKey)
	}

	if len(extendData.Attachments) > 0 {
		// 是否有副表
		for _, attach := range extendData.Attachments {
			var (
				attrData       g.MapStrStr
				attrExtendData *model.ToolsGenTableEx
			)
			attach.ParentTable = append(attach.ParentTable, extendData.BusinessName)
			attrData, attrExtendData, err = s.GenData(ctx, attach.TableId, attach)
			liberr.ErrIsNil(ctx, err)
			extendData.AttachmentList = append(extendData.AttachmentList, &model.AttachmentList{
				Data: attrData,
				Attr: attrExtendData,
			})
		}
...
func (s *sToolsGenTable) SelectRecordById(ctx context.Context, tableId int64) (tableEx *model.ToolsGenTableEx, err error) {
	...
	for _, column := range allColumnExs {
		...
		allColumnMap, err2 := service.ToolsGenTableColumn().SelectDbTableColumnMapByName(ctx, linkedTableName, table.Group) //关联表只能是同数据库group的
		if err2 != nil {
			err = err2
			return
		}
...
func (s *sToolsGenTable) GenCode(ctx context.Context, ids []int) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		for _, id := range ids {
			...
			genData, extendData, err = s.GenData(ctx, gconv.Int64(id), nil)
			liberr.ErrIsNil(ctx, err)

			// 栈元素：记录当前节点 + 到当前节点为止的 IsAttachment 路径（包含当前）
			type stackItem struct {
				node *model.AttachmentList
				path []string // 从根到当前节点（含）的 IsAttachment 值
			}
			...
			stack := []stackItem{
				{node: &model.AttachmentList{
					Attr: extendData,
					Data: genData,
				}, path: []string{gstr.CaseCamelLower(extendData.BusinessName)}},
			}
			...
			for len(stack) > 0 {
				...
				// ✅ 此时 path 就是从根到当前节点的所有 IsAttachment（顺序从根到当前）
				fmt.Printf("Node IsAttachment=%v, Full parent path (root→current): %v\n",
					current.Attr.IsAttachment, path)

				packageName := current.Attr.PackageName
				businessName := gstr.CaseCamelLower(current.Attr.BusinessName)
				modulePath := gstr.StrEx(current.Attr.PackageName, "internal/app/")
				overwriteInfo := gmap.NewStrAnyMap()
				for _, ov := range current.Attr.OverwriteInfo {
					overwriteInfo.Set(ov.Key, ov.Value)
				}
				pathStr := strings.Join(path, "/")
				for key, code := range current.Data {
					switch key {
					case "api":
						path := strings.Join([]string{curDir, "/", apiName, "/", modulePath, "/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("api")))
						liberr.ErrIsNil(ctx, err)
					case "controller":
						path := strings.Join([]string{curDir, "/", packageName, "/controller/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("controller")))
						liberr.ErrIsNil(ctx, err)
					case "dao":
						path := strings.Join([]string{curDir, "/", packageName, "/dao/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("dao")))
						liberr.ErrIsNil(ctx, err)
					case "dao_internal":
						path := strings.Join([]string{curDir, "/", packageName, "/dao/internal/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("dao_internal")))
						liberr.ErrIsNil(ctx, err)
					case "logic":
						path := strings.Join([]string{curDir, "/", packageName, "/logic/", businessName, "/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("logic")))
						liberr.ErrIsNil(ctx, err)
					case "model":
						path := strings.Join([]string{curDir, "/", packageName, "/model/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("model")))
						liberr.ErrIsNil(ctx, err)
					case "model_do":
						path := strings.Join([]string{curDir, "/", packageName, "/model/do/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("model_do")))
						liberr.ErrIsNil(ctx, err)
					case "model_entity":
						path := strings.Join([]string{curDir, "/", packageName, "/model/entity/", current.Attr.BusinessName, ".go"}, "")
					...
					case "router_func":
						path := strings.Join([]string{curDir, "/", packageName, "/router/", current.Attr.BusinessName, ".go"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("router_func")))
						liberr.ErrIsNil(ctx, err)
					case "service":
						path := strings.Join([]string{curDir, "/", packageName, "/service/", current.Attr.BusinessName, ".go"}, "")
					...
					case "sql":
						...
						path := strings.Join([]string{curDir, "/resource/data/gen_sql/", modulePath, "/", current.Attr.BusinessName, ".sql"}, "")
					...
					case "tsApi":
						path := strings.Join([]string{frontDir, "/src/api/", modulePath, "/", pathStr, ".ts"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("tsApi")))
						liberr.ErrIsNil(ctx, err)
					case "tsModel":
						path := strings.Join([]string{frontDir, "/src/views/", modulePath, "/", pathStr + "/list/component/model", ".ts"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("tsModel")))
						liberr.ErrIsNil(ctx, err)
					case "vue":
						path := strings.Join([]string{frontDir, "/src/views/", modulePath, "/", pathStr, "/list/index.vue"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("vue")))
						liberr.ErrIsNil(ctx, err)
					case "vueDetail":
						path := strings.Join([]string{frontDir, "/src/views/", modulePath, "/", pathStr + "/list/component/detail", ".vue"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("vueDetail")))
						liberr.ErrIsNil(ctx, err)
					case "vueEdit":
						path := strings.Join([]string{frontDir, "/src/views/", modulePath, "/", pathStr + "/list/component/edit", ".vue"}, "")
						err = s.createFile(path, code, gconv.Bool(overwriteInfo.Get("vueEdit")))
						liberr.ErrIsNil(ctx, err)
					}
				}
				//生成模块路由
				err = s.genModuleRouter(curDir, goModName, current.Attr.ModuleName, modulePath)
				liberr.ErrIsNil(ctx, err)
				//生成模块boot logic
				err = s.genModuleBootLogic(curDir, current.Attr.ModuleName, modulePath)
				liberr.ErrIsNil(ctx, err)
				//生成对应模块的业务logic
				err = s.genModuleLogic(curDir, goModName, current.Attr.PackageName)
				liberr.ErrIsNil(ctx, err)

				// 遍历所有 AttachmentList 的 Attr
				for _, att := range current.Attr.AttachmentList {
					if att != nil && att.Attr != nil {
						// 新路径 = 当前 path + 子节点的 IsAttachment
						newPath := make([]string, len(path)+1)
						copy(newPath, path)
						newPath[len(path)] = gstr.CaseCamelLower(att.Attr.BusinessName)
...
func (s *sToolsGenTable) SyncTable(ctx context.Context, tableId int64) (err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			...
			genTableColumns, err = service.ToolsGenTableColumn().SelectDbTableColumnsByName(ctx, extendData.TableName, extendData.Group)
			liberr.ErrIsNil(ctx, err, "获取列数据失败")
			if len(genTableColumns) <= 0 {
				liberr.ErrIsNil(ctx, gerror.New("获取列数据失败"))
			}

...
type ToolsGenTableEx struct {
	...
	Attachments    []*entity.Attachments    //附加表
	...
	MenuPid        uint                     // 父级菜单ID
	IsAttachment   bool                     // 当前表是否为副表
	AttachmentList []*AttachmentList        // 附加表信息
}

// AttachmentList 附加表生成代码数据
type AttachmentList struct {
	Data g.MapStrStr
	Attr *ToolsGenTableEx
}

// ToolsGenTableLinked 关联表
type ToolsGenTableLinked struct {
	// 这里不加group默认和主表同一个group
	TableName     string                 // 表名称
	TableComment  string                 // 表描述
	ClassName     string                 // 实体类名称
	PackageName   string                 // 生成包路径
